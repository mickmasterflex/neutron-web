    ...
    <div :class="$attrs.field_wrap_class">
      <div class="relative w-64">
        ...
        <select
          ...
          :id="$attrs.field_id">
          <option value="">----------</option>
          <option class="capitalize" :selected="option.id === value ? 'selected' : false" v-for="option in $attrs.options" :key="option.id" :value="`${option.id}`">{{option.name.charAt(0).toUpperCase() + option.name.slice(1)}}</option>
        </select>
