    ...
    <template #content>
      <div class="space-x-4 mb-3">
          <span v-if="content.additional_content_type === 'tcpa'">
            Leadid Toggle: <span class="font-bold" :class="content.leadid_toggle ? 'text-green-600' : 'text-red-500'">{{ content.leadid_toggle ? content.leadid_toggle_field_name : content.leadid_toggle }}</span>
          </span>
        <span>
          Double Opt-in: <span class="font-bold" :class="content.double_optin ? 'text-green-600' : 'text-red-500'">{{ content.double_optin ? content.double_optin_field_name : content.double_optin }}</span>
        </span>
      </div>
